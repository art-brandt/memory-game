...
var tryGame = function() {
    $(".card").on("click", function(event){
        ...
        let clickedCards = $('.card').filter('[data-tid="Card-flipped"]');
        if (clickedCards.length == 2) {
            ...
            if (firstClickedCardName == secondClickedCardName) {
                setTimeout(function(){ 
                    $(clickedCards[0]).addClass("empty-card").css("background-image","none").removeAttr("data-tid").removeAttr("data-name");
                    $(clickedCards[1]).addClass("empty-card").css("background-image","none").removeAttr("data-tid").removeAttr("data-name");
                ...
                setTimeout(function(){
                    score = score - $(".card").filter(".back").length * 42;
                    $(".score").text(score);
                    $(clickedCards[0]).attr("data-tid","Card").addClass('back');
                    $(clickedCards[1]).attr("data-tid","Card").addClass('back');
                    tryGame();
                }, 1400);
            }
        }
...
$(document).ready(function() {
    for (let i = 0; i < doubleDeck.length; i++) {
        $('.cards-container').append('<div class="card" data-name="' + objCards[i].name +'" data-tid="Card"></div');
